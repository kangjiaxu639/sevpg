...
from habitat_baselines.rl.representation.multi_distill import MultiDistill
...
from habitat_baselines.common.semantic_mapping import Semantic_Mapping
...
from habitat_baselines.common.map_utils.depth_utils import _visualize_map
...
class DDPPOTrainer_RewardActPred(PPOTrainer_RewardActPred):
    ...
    def train(self) -> None:
        ...
        num_episodes = self.config.NUM_UPDATES * ppo_cfg.num_steps / self.config.ENV_CONFIG.ENVIRONMENT.MAX_EPISODE_STEPS
        if distill_cfg.DINO:
            self.distill_agent = MultiDistill(
                distill_cfg,
                self.device,
                int(num_episodes),
                int(self.config.ENV_CONFIG.ENVIRONMENT.MAX_EPISODE_STEPS / ppo_cfg.num_steps)
            )
            self.distill_agent = DistributedDataParallel(
                self.distill_agent, device_ids=[self.local_rank],
                output_device=self.local_rank, broadcast_buffers=True
            )
            ...
            if self.world_rank == 0:
                logger.info(
                    "idm number of trainable parameters: {}".format(
                        sum(
                            param.numel()
                            for param in self.distill_agent.parameters()
                            if param.requires_grad
                        )
                    )
                )
################################################################################################
        ...
        nc = threedmap_cfg.num_sem_categories + 4
        self.local_map = torch.zeros(self.config.NUM_PROCESSES, nc, self.local_w,
...
##############################################################################################
        ...
        _, self.local_map, _, self.local_pose, self.observation_points = \
            self.sem_map_module(obs_after_process, poses, self.local_map, self.local_pose, self.origins,
        ...
        with (
            ...
            for update in range(start_update, self.config.NUM_UPDATES):
                ...
                input = {
                    ...
                    "num_sem_categories": threedmap_cfg.num_sem_categories,
                    "sem_map_pred": self.local_map[0, 4:, :, :].argmax(0).cpu().detach().numpy(),
                    "rgb_vis": self.rgb_vis,
                }
                _visualize_map(input)
                ...
                if self.config.SAVE_PPO_IMAGE:
                    image_index_initial = self.save_image(rollouts, image_index_initial) # save image from gibsion
                ...
                if distill_cfg.DINO:
                    observations = rollouts.observations.permute((1, 0, 4, 2, 3))[0]
                    observations = transforms.RandomResizedCrop(16, scale=(0.4, 1.0),
                                         interpolation=Image.BICUBIC)(observations)
                    obs_points = rollouts.obs_points.permute((1, 0, 2, 3))[0]
                    obs_points = obs_points.view((obs_points.shape[0],obs_points.shape[1], 64, 64))[:, :3]
                    print(
                        "observations shape:{0}, obs_points shape:{1}".format(
                            observations.shape, obs_points.shape))
                    multi_distill_loss = self.distill_agent.module.updata_distill(
                        observations, obs_points,\
                        distill_cfg.clip_grad, distill_cfg.freeze_last_layer, update
                        )
                    # average across multiple gpus
                    multi_distill_loss = comm.reduce_dict(multi_distill_loss)

                    if self.world_rank == 0:
                        writer.add_scalar("multi_distill_loss",
                                          multi_distill_loss["multi_distill_loss"],
                                          count_steps)
                ...
                with torch.no_grad():
                    if rl_cfg.rnd:
                        ...
                        intr_reward = self.reward_agent.module.compute_simclr_reward(rollouts.observations[1:])
