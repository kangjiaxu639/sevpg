...
def train_dino(args):
    ...
    # if the network is a XCiT
        ...
        teacher = torch.hub.load('facebookresearch/xcit:main', args.arch,
                                 pretrained=False)
        embed_dim = student.embed_dim
    ...
    image = torch.randint(low=0, high=255, size=(17, 3, 120, 160)).float().to("cuda:0")
    points = torch.randint(low=0, high=255, size=(17, 3, 64, 64)).float().to("cuda:0")
    student, teacher = student.cuda(), teacher.cuda()
    out = student(image)
    point_emb = teacher(points)
    print(out.shape, point_emb.shape)
    ...
    #     args.lr * (
    #             args.batch_size_per_gpu * utils.get_world_size()) / 256.,
    #     # linear scaling rule
    #     args.min_lr,
    #     args.epochs, step,
    ...
    #                               dino_loss,
    #                               data_loader, optimizer, lr_schedule,
    ...
    #                      os.path.join(args.output_dir, 'checkpoint.pth'))
    # if args.saveckp_freq and step % args.saveckp_freq == 0:
...
# def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, images,
#                     optimizer, lr_schedule, wd_schedule, momentum_schedule,epoch,
#                     step, fp16_scaler, args):
...
#     with torch.no_grad():
#         m = momentum_schedule[step m]  # momentum parameter
