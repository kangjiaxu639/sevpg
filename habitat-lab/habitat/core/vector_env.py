...
class SimpleVectorEnv:
    ...
    def _worker_env(
        ...
        try:
            command, data = connection_read_fn()
            while command != CLOSE_COMMAND and command != EPISODE_COMMAND and command != COUNT_EPISODES_COMMAND and command != EPISODE_OVER and command != GET_METRICS:
                if command == STEP_COMMAND:
                    # different step methods for habitat.RLEnv and habitat.Env
                    if isinstance(env, habitat.RLEnv) or isinstance(
                        env, gym.Env
                        ...
                        # habitat.RLEnv
                        obs_after_process, observations, reward, done, info = env.step(**data)
                        if auto_reset_done and done:
                            obs_after_process, observations, info = env.reset()
                        connection_write_fn((obs_after_process, observations, reward, done, info))
                    elif isinstance(env, habitat.Env):
                        # habitat.Env
                        observations = env.step(**data)
                        if auto_reset_done and env.episode_over:
    ...
    def async_step(self, data: List[Union[int, str, Dict[str, Any]]]) -> None:
        ...
        # Backward compatibility
        if isinstance(data[0], (int, np.integer, str)):
            data = [{"action": action} for action in data]
